	...
	for ( i=0 ; i<GPtr->numExtents ; i++ )
	{
		...
		if ( extentStartBlock >= maxNABlks )
		{
			*lastExtentIndex = i;
			...
			if (fsck_get_verbosity_level() >= kDebugLog) {
				fsck_print(ctx, LOG_TYPE_INFO, "\tCheckExtRecord: id=%u %d:(%u,%u), maxBlocks=%u (startBlock > maxBlocks)\n",
		...
		if ( extentBlockCount >= (maxNABlks - extentStartBlock) ) 
		{
			*lastExtentIndex = i;
			RcdError( GPtr, E_ExtEnt );
			if (fsck_get_verbosity_level() >= kDebugLog) {
				fsck_print(ctx, LOG_TYPE_INFO, "\tCheckExtRecord: id=%u %d:(%u,%u), maxBlocks=%u (blockCount > (maxBlocks - startBlock))\n",
						fileID, i, extentStartBlock, extentBlockCount, maxNABlks);
			}
			return( E_ExtEnt );
		}			
		     ...
		     ((extentStartBlock == 0) && (extentBlockCount != 0)))
		{
			*lastExtentIndex = i;
			RcdError( GPtr, E_ExtEnt );
			if (fsck_get_verbosity_level() >= kDebugLog) {
				fsck_print(ctx, LOG_TYPE_INFO, "\tCheckExtRecord: id=%u %d:(%u,%u), (startBlock == 0)\n",
		...
		if ((extentStartBlock != 0) && (extentBlockCount == 0))
		{
			*lastExtentIndex = i;
			RcdError( GPtr, E_ExtEnt );
			if (fsck_get_verbosity_level() >= kDebugLog) {
				fsck_print(ctx, LOG_TYPE_INFO, "\tCheckExtRecord: id=%u %d:(%u,%u), (blockCount == 0)\n",
						fileID, i, extentStartBlock, extentBlockCount);
			}
			return( E_ExtEnt );
		}	
...
BTCheck(SGlobPtr GPtr, short refNum, CheckLeafRecordProcPtr checkLeafRecord)
{
	...
	{
		if (state.debug)
			fsck_print(ctx, LOG_TYPE_INFO, "Header root node %u, calculated total nodes %u, tree depth %u, header node num %u\n",
		...
		for (i = 0; i < kHFSPlusExtentDensity; i++) {
			if (((*extp)[i].blockCount % blocksPerNode) != 0) {
				...
				*statusFlag |= S_RebuildBTree;
				fsckPrintFormat(GPtr->context, E_BTreeSplitNode, calculatedBTCB->fcbPtr->fcbFileID);
				if (state.debug == 0) {
					goto exit;
				} else {
					fsck_print(ctx, LOG_TYPE_INFO, "Improperly split node in file id %u, offset %u (extent #%d), Extent <%u, %u>\n", calculatedBTCB->fcbPtr->fcbFileID, totalBlocks, i, (*extp)[i].startBlock, (*extp)[i].blockCount);
				}
			}
			...
			while (noErr == (err = BTIterateRecord(GPtr->calculatedExtentsFCB, kBTreeNextRecord, &iterator, &btRecord, &recordSize))) {
				...
				for (i = 0; i < kHFSPlusExtentDensity; i++) {
					if ((extRecord[i].blockCount % blocksPerNode) != 0) {
						result = errRebuildBtree;
						*statusFlag |= S_RebuildBTree;
						fsckPrintFormat(GPtr->context, E_BTreeSplitNode, fileID);
						if (state.debug == 0) {
							goto exit;
						} else {
							fsck_print(ctx, LOG_TYPE_INFO, "Improperly split node in file id %u, startBlock %u, index %d (offset %u), extent <%u, %u>\n", fileID, key->startBlock, i, totalBlocks, extRecord[i].startBlock, extRecord[i].blockCount);
						}
					}
	...
	fsck_print(ctx, LOG_TYPE_INFO, "    treeDepth     = %d \n", header->treeDepth );
	fsck_print(ctx, LOG_TYPE_INFO, "    rootNode      = %d \n", header->rootNode );
	fsck_print(ctx, LOG_TYPE_INFO, "    leafRecords   = %d \n", header->leafRecords );
	fsck_print(ctx, LOG_TYPE_INFO, "    firstLeafNode = %d \n", header->firstLeafNode );
	fsck_print(ctx, LOG_TYPE_INFO, "    lastLeafNode  = %d \n", header->lastLeafNode );
	fsck_print(ctx, LOG_TYPE_INFO, "    totalNodes    = %d \n", header->totalNodes );
	fsck_print(ctx, LOG_TYPE_INFO, "    freeNodes     = %d \n", header->freeNodes );
#endif
	...
	while ( GPtr->BTLevel > 0 )
	{
		...
		if ( index < 0 )
		{
			...
			if ( nodeDescP->kind == kBTLeafNode ) 
			{
				...
				fsck_print(ctx, LOG_TYPE_INFO, "%d ", nodeNum );
			...
			if ( nodeDescP->bLink != tprP->TPRLtSib )
			{
				...
				if (state.debug)
					fsck_print(ctx, LOG_TYPE_INFO, "Node %d's back link is 0x%x; expected 0x%x\n"
			...
			else
			{
				...
				{				
					...
					RcdError( GPtr, E_SibLk );
					if (state.debug)
						fsck_print(ctx, LOG_TYPE_INFO, "Node %d's forward link is 0x%x; expected 0x%x\n"
							   "    disk offset = 0x%llx, size = 0x%x\n",
			...
			if ( nodeDescP->height != calculatedBTCB->treeDepth - GPtr->BTLevel + 1 )
			{
				result = E_NHeight;
				RcdError( GPtr, E_NHeight );
				if (!state.debug) goto RebuildBTreeExit;
			}
			
			if (result && (state.cur_debug_level & d_dump_node))
			{
			...
			if ( hasParKey == true )
			{
				...
				if ( CompareKeys( (BTreeControlBlockPtr)calculatedBTCB, (BTreeKey *)parKey, keyPtr ) != 0 )
				{
					if (state.debug)
					{
						fsck_print(ctx, LOG_TYPE_INFO, "Index key doesn't match first node key\n");
						if (state.cur_debug_level & d_dump_record)
						{
							fsck_print(ctx, LOG_TYPE_INFO, "Found (child; node %u):\n", tprP->TPRNodeN);
							HexDump(keyPtr, CalcKeySize(calculatedBTCB, keyPtr), FALSE);
							fsck_print(ctx, LOG_TYPE_INFO, "Expected (parent; node %u):\n", tprP[-1].TPRNodeN);
	...
	while ( mapSize > 0 )
	{
		...
		if ( nodeNum != 0 )
		{
			...
			if ( nodeDescP->kind != kBTMapNode )
			{
				...
				if (state.debug)
					fsck_print(ctx, LOG_TYPE_INFO, "Expected map node, got type %d\n", nodeDescP->kind);
				result = E_BadMapN;
				goto exit;
			}	
			if ( nodeDescP->numRecords != Num_MRecs )
			{
				RcdError( GPtr, E_BadMapN );
				if (state.debug)
					fsck_print(ctx, LOG_TYPE_INFO, "Expected %d records in node, found %d\n", Num_MRecs, nodeDescP->numRecords);
				result = E_BadMapN;
				goto exit;
			}	
			if ( nodeDescP->height != 0 )
...
OSErr	CmpBTH( SGlobPtr GPtr, SInt16 fileRefNum )
{
	...
	if (calculatedBTCB->leafRecords != bTreeHeader.leafRecords) {
		...
		printMsg = 1;
		fsckPrintFormat(GPtr->context, E_LeafCnt);
	...
	if (isBTHDamaged || printMsg) {
    		*statP = *statP | S_BTH;
		if (isBTHDamaged) {
			fsckPrintFormat(GPtr->context, E_InvalidBTreeHeader);
		}
	}
	return( noErr );
}
				...
				if ( CompareKeys( (BTreeControlBlockPtr)btcb, prevkeyP, keyPtr ) >= 0 )
				{
					    ...
					    (CompareKeys(btcb, prevkeyP, (KeyPtr)&gMetaDataDirKey) == 0))
					{
						if (fsck_get_verbosity_level() > 0)
							fsck_print(ctx, LOG_TYPE_INFO, "Problem: b-tree key for \"HFS+ Private Data\" directory is out of order.\n");
					...
					else
					{
                        ...
                        if (fsck_get_verbosity_level() > 0)
                            fsck_print(ctx, LOG_TYPE_INFO, "Records %d and %d (0-based); offsets 0x%04X and 0x%04X\n", index-1, index, (long)prevkeyP - (long)nodeP, (long)keyPtr - (long)nodeP);
						result = E_KeyOrd;
					}
				}
			}
	...
	if (result == E_KeyOrd)
	{
		if (state.cur_debug_level & d_dump_record)
		{
			for (index = 0; index < nodeP->numRecords; ++index)
			{
				...
				fsck_print(ctx, LOG_TYPE_INFO, "Record %d (offset 0x%04X):\n", index, (long)keyPtr - (long)nodeP);
		...
		if (state.cur_debug_level & d_dump_node)
		{
			fsck_print(ctx, LOG_TYPE_INFO, "Node:\n");
...
int CmpMDB( SGlobPtr GPtr,  HFSMasterDirectoryBlock * mdbP)
{
	...
	if ( mdbP->drSigWord	!= vcb->vcbSignature ) {
		if ( fsck_get_verbosity_level() >= kDebugLog )
			fsck_print(ctx, LOG_TYPE_INFO, "\tinvalid MDB drSigWord \n" );
		isMDBDamaged = 1;
	}	
	if ( mdbP->drCrDate	!= vcb->vcbCreateDate )	{
		if ( fsck_get_verbosity_level() >= kDebugLog )
			fsck_print(ctx, LOG_TYPE_INFO, "\tinvalid MDB drCrDate \n" );
		isMDBDamaged = 1;
	}	
	if ( mdbP->drLsMod	!= vcb->vcbModifyDate )	{
		if ( fsck_get_verbosity_level() >= kDebugLog )
			fsck_print(ctx, LOG_TYPE_INFO, "\tinvalid MDB drLsMod \n" );
		isMDBDamaged = 1;
	}	
	if ( mdbP->drAtrb	!= (UInt16)vcb->vcbAttributes )	{
		if ( fsck_get_verbosity_level() >= kDebugLog )
			fsck_print(ctx, LOG_TYPE_INFO, "\tinvalid MDB drAtrb \n" );
		isMDBDamaged = 1;
	}	
	if ( mdbP->drVBMSt	!= vcb->vcbVBMSt )	{
		if ( fsck_get_verbosity_level() >= kDebugLog )
			fsck_print(ctx, LOG_TYPE_INFO, "\tinvalid MDB drVBMSt \n" );
		isMDBDamaged = 1;
	}	
	if ( mdbP->drNmAlBlks	!= vcb->vcbTotalBlocks ) {
		if ( fsck_get_verbosity_level() >= kDebugLog )
			fsck_print(ctx, LOG_TYPE_INFO, "\tinvalid MDB drNmAlBlks \n" );
		isMDBDamaged = 1;
	}	
	if ( mdbP->drClpSiz	!= vcb->vcbDataClumpSize )	{
		if ( fsck_get_verbosity_level() >= kDebugLog )
			fsck_print(ctx, LOG_TYPE_INFO, "\tinvalid MDB drClpSiz \n" );
		isMDBDamaged = 1;
	}	
	if ( mdbP->drAlBlSt	!= vcb->vcbAlBlSt )	{
		if ( fsck_get_verbosity_level() >= kDebugLog )
			fsck_print(ctx, LOG_TYPE_INFO, "\tinvalid MDB drAlBlSt \n" );
		isMDBDamaged = 1;
	}	
	if ( mdbP->drNxtCNID	!= vcb->vcbNextCatalogID )	{
		if ( fsck_get_verbosity_level() >= kDebugLog )
			fsck_print(ctx, LOG_TYPE_INFO, "\tinvalid MDB drNxtCNID \n" );
		isMDBDamaged = 1;
	}	
	if ( CmpBlock( mdbP->drVN, vcb->vcbVN, mdbP->drVN[0]+1 ) )	{
		if ( fsck_get_verbosity_level() >= kDebugLog )
			fsck_print(ctx, LOG_TYPE_INFO, "\tinvalid MDB drVN \n" );
		isMDBDamaged = 1;
	}	
	if ( mdbP->drVolBkUp	!= vcb->vcbBackupDate )	{
		if ( fsck_get_verbosity_level() >= kDebugLog )
			fsck_print(ctx, LOG_TYPE_INFO, "\tinvalid MDB drVolBkUp \n" );
		isMDBDamaged = 1;
	}	
	if ( mdbP->drVSeqNum	!= vcb->vcbVSeqNum )	{
		if ( fsck_get_verbosity_level() >= kDebugLog )
			fsck_print(ctx, LOG_TYPE_INFO, "\tinvalid MDB drVSeqNum \n" );
		isMDBDamaged = 1;
	}	
	if ( mdbP->drWrCnt	!= vcb->vcbWriteCount )	{
		if ( fsck_get_verbosity_level() >= kDebugLog )
			fsck_print(ctx, LOG_TYPE_INFO, "\tinvalid MDB drWrCnt \n" );
		isMDBDamaged = 1;
	}	
	if ( mdbP->drXTClpSiz	!= vcb->vcbExtentsFile->fcbClumpSize )	{
		if ( fsck_get_verbosity_level() >= kDebugLog )
			fsck_print(ctx, LOG_TYPE_INFO, "\tinvalid MDB drXTClpSiz \n" );
		isMDBDamaged = 1;
	}	
	if ( mdbP->drCTClpSiz	!= vcb->vcbCatalogFile->fcbClumpSize )	{
		if ( fsck_get_verbosity_level() >= kDebugLog )
			fsck_print(ctx, LOG_TYPE_INFO, "\tinvalid MDB drCTClpSiz \n" );
		isMDBDamaged = 1;
	}	
	if ( mdbP->drNmRtDirs	!= vcb->vcbNmRtDirs )	{
		if ( fsck_get_verbosity_level() >= kDebugLog )
			fsck_print(ctx, LOG_TYPE_INFO, "\tinvalid MDB drNmRtDirs \n" );
		isMDBDamaged = 1;
	}	
	if ( mdbP->drFilCnt	!= vcb->vcbFileCount )	{
		if ( fsck_get_verbosity_level() >= kDebugLog )
			fsck_print(ctx, LOG_TYPE_INFO, "\tinvalid MDB drFilCnt \n" );
		isMDBDamaged = 1;
	}	
	if ( mdbP->drDirCnt	!= vcb->vcbFolderCount )	{
		if ( fsck_get_verbosity_level() >= kDebugLog )
			fsck_print(ctx, LOG_TYPE_INFO, "\tinvalid MDB drDirCnt \n" );
		isMDBDamaged = 1;
	}	
	if ( CmpBlock(mdbP->drFndrInfo, vcb->vcbFinderInfo, 32 ) )	{
		if ( fsck_get_verbosity_level() >= kDebugLog )
			fsck_print(ctx, LOG_TYPE_INFO, "\tinvalid MDB drFndrInfo \n" );
		isMDBDamaged = 1;
	}	
...
OSErr CompareVolumeHeader( SGlobPtr GPtr, HFSPlusVolumeHeader *volumeHeader )
{
	...
	if ( volumeHeader->fileCount != vcb->vcbFileCount && 
		...
		fsckPrintFormat(GPtr->context, E_FilCnt);
	...
	if ( volumeHeader->folderCount != vcb->vcbFolderCount && 
		 (GPtr->CatStat & S_Valence) == 0 ) {
		fsckPrintFormat(GPtr->context, E_DirCnt);
	...
	if (volumeHeader->freeBlocks != vcb->vcbFreeBlocks) {
		fsckPrintFormat(GPtr->context, E_FreeBlocks);
	...
	if ( volumeHeader->catalogFile.clumpSize != vcb->vcbCatalogFile->fcbClumpSize ) {
		fsckPrintFormat(GPtr->context, E_InvalidClumpSize);
	...
	if ( volumeHeader->signature != kHFSPlusSigWord  &&
		...
		if ( fsck_get_verbosity_level() >= kDebugLog )
			fsck_print(ctx, LOG_TYPE_INFO, "\tinvalid VHB signature \n" );
		isVHDamaged = 1;
	}
