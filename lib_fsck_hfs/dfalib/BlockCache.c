...
ReadFragmentedBlock (SFCB *file, UInt32 blockNum, BlockDescriptor *block)
{
	...
	for (i = 0; (i < maxFrags) && (blockSize > 0); ++i) {
		...
		if (bufs[i]->Length != fragSize) {
            if (state.debug) {
                fsck_print(ctx, LOG_TYPE_INFO, "ReadFrstate.debugLength != fragSize)\n");
            }
			result = -1;
			goto ErrorExit;
		}
...
WriteFragmentedBlock( SFCB *file, BlockDescriptor *block, int age, uint32_t writeOptions )
{
	...
	if (bufs == NULL) {
        if (state.debug) {
            fsck_print(ctx, LOG_TYPE_INFO, "WriteFragmentedBlock: NULL bufs list!\n");
        }
		return (-1);
	}
...
ReleaseFragmentedBlock (SFCB *file, BlockDescriptor *block, int age)
{
	...
	if (bufs == NULL) {
        if (state.debug) {
            fsck_print(ctx, LOG_TYPE_INFO, "ReleaseFragmentedBlock: NULL buf list!\n");
        }
		return (-1);
	}
