...
int BitMapCheckEnd(void)
{
	if (gBitMapInited) {
		...
		fsck_print(ctx, LOG_TYPE_INFO, "   %d full segments, %d segment nodes (max depth was %d nodes)\n",
...
static int GetSegmentBitmap(UInt32 startBit, UInt32 **buffer, int bitOperation)
{
	...
	*buffer = NULL;
	...
	if (*buffer == NULL) {
#if _VBC_DEBUG_
		fsck_print(ctx, LOG_TYPE_INFO, "GetSegmentBitmap: couldn't get a node for block %d, segment %d\n", startBit, segment);
#endif
		return (-1); /* oops */
	}
	...
	if (segNode) {
		int i;
		fsck_print(ctx, LOG_TYPE_INFO, "  segment %d: L=0x%08x, R=0x%08x \n< ",
			(int)segNode->segment, (int)segNode->left, segNode->right);
		for (i = 0; i < kWordsPerSegment; ++i) {
			fsck_print(ctx, LOG_TYPE_INFO, "0x%08x ", segNode->bitmap[i]);
			if ((i & 0x3) == 0x3)
	...
	if (bitOperation == kSettingBits && *buffer && bcmp(*buffer, gFullBitmapSegment, kBytesPerSegment) == 0) {
		fsck_print(ctx, LOG_TYPE_INFO, "*** segment %d (start blk %d) is already full!\n", segment, startBit);
		exit(5);
	}
	if (bitOperation == kClearingBits && *buffer && bcmp(*buffer, gEmptyBitmapSegment, kBytesPerSegment) == 0) {
		fsck_print(ctx, LOG_TYPE_INFO, "*** segment %d (start blk %d) is already empty!\n", segment, startBit);
		exit(5);
	}
#endif
...
int CaptureBitmapBits(UInt32 startBit, UInt32 bitCount)
{
	...
	while (bitCount >= kBitsPerWord) {
		...
		if (SWAP_BE32(*currentWord) & bitMask) {
			...
			//fsck_print(ctx, LOG_TYPE_INFO, "(2) overlapping file blocks! word: 0x%08x, mask: 0x%08x\n", *currentWord, bitMask);
		}
	...
	if (bitCount != 0) {
		...
		if (SWAP_BE32(*currentWord) & bitMask) {
			...
			//fsck_print(ctx, LOG_TYPE_INFO, "(3) overlapping file blocks! word: 0x%08x, mask: 0x%08x\n", *currentWord, bitMask);
		}
...
int CheckVolumeBitMap(SGlobPtr g, Boolean repair)
{
	...
	for (bit = 0; bit < gTotalBits; bit += kBitsPerSegment) {
		...
		if (repair) {
			...
			fsck_print(ctx, LOG_TYPE_INFO, "  disk buffer + %d\n", (bit & bitsWithinFileBlkMask)/8);
			fsck_print(ctx, LOG_TYPE_INFO, "start block number for segment = %qu\n", bit);
			fsck_print(ctx, LOG_TYPE_INFO, "segment %qd\n", bit / kBitsPerSegment);

			fsck_print(ctx, LOG_TYPE_INFO, "Memory:\n");
			for (i = 0; i < kWordsPerSegment; ++i) {
				fsck_print(ctx, LOG_TYPE_INFO, "0x%08x ", buffer[i]);
				if ((i & 0x7) == 0x7)
					fsck_print(ctx, LOG_TYPE_INFO, "\n");
			}

			disk_buffer = (UInt32*) (vbmBlockP + (bit & bitsWithinFileBlkMask)/8);
			fsck_print(ctx, LOG_TYPE_INFO, "Disk:\n");
			for (i = 0; i < kWordsPerSegment; ++i) {
				fsck_print(ctx, LOG_TYPE_INFO, "0x%08x ", disk_buffer[i]);
				if ((i & 0x7) == 0x7)
					fsck_print(ctx, LOG_TYPE_INFO, "\n");
			}
				...
				if (buffer[i] != disk_buffer[i]) {
					...
					for (j = 0; j < kBitsPerWord; ++j) {
						...
						if ((buffer[i] & dummy) != (disk_buffer[i] & dummy)) {
							...
							if (buffer[i] & dummy) {
								fsck_print(ctx, LOG_TYPE_INFO, "Allocation block %u should be marked used on disk.\n", block_num);
							} else {
								fsck_print(ctx, LOG_TYPE_INFO, "Allocation block %u should be marked free on disk.\n", block_num);
							}
						}
						dummy = dummy >> 1;
					}
...
static void TrimFlush(void)
{
	...
	{
        fsck_debug_print(ctx, d_info|d_trim, "TrimFlush: nothing to flush\n");
		return;	
	}
	
    // TODO: rdar://102452908 (lib_fsck_hfs remove UNMAP ioctls)
	err = ioctl(state.fsreadfd, DKIOCUNMAP, &gTrimData);
	if (err == -1)
	{
...
static void TrimExtent(SGlobPtr g, UInt32 startBlock, UInt32 blockCount)
{
    ...
    fsck_debug_print(ctx, d_info|d_trim, "Trimming: startBlock=%10u, blockCount=%10u\n", startBlock, blockCount);
...
int IsTrimSupported(void)
{
    ...
    // TODO: rdar://102452908 (lib_fsck_hfs remove UNMAP  ioctls)
	err = ioctl(state.fsreadfd, DKIOCGETFEATURES, &features);
	if (err < 0)
	{
...
BMS_PrintTree(BMS_Node * root)
{
	if (root) {
		...
		fsck_print(ctx, LOG_TYPE_INFO, "seg %d\n", root->segment);
